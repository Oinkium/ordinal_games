...
\begin{theorem}
  ...
  \begin{proof}
    ...
    It remains to show that $\comp\tau\sigma$ is alternating with respect to $\zeta_{A\implies C}$.  Let $s\in\comp\tau\sigma$, and let $\s\in\sigma\|\tau$ be minimal such that $\s\vert_{A,C}=s$ (so if $s=\emptyplay$ then $\s=\emptyplay$ and if $s=s'y$ then $\s$ has $y$ as its last move).  

    Suppose first that $sa\in\sigma$, where $a\in M_A$.  We have $\lambda_{A\implies C}(a)=$
  \end{proof}
\end{theorem}
