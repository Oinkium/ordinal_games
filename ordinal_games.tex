...
\begin{proposition}
  ...
  \begin{proof}
    ...
    There is no distinction between the set $P_X$ of plays and the set $L_X$ of legal plays for any negative game $X$.  Since $\sigma\subset P_{A\implies B}$ and $\tau\subset P_{B\implies C}$, Lemma \ref{alternatingsequenceslemma} tells us that $\s\vert_{A,C}\in P_{A\implies C}$.  
    ...
    We shall use the argument given in \cite{abramskyjagadeesangames}.  Given a sequence $s\in\comp\tau\sigma$, we say that $\s\in\sigma\|\tau$ \emph{covers} $s$ if $s\prefix\s\vert_{A,C}$.  By definition, every sequence in $\comp\tau\sigma$ is covered by some sequence in $\comp\tau\sigma$.  We make two claims:
    
    i) If $s\in\comp\tau\sigma$, then there is a sequence $\s$ that is minimal among sequences covering $s$ (with respect to the prefix ordering) - in such a case, we must have $s=\s\vert_{A,C}$.

    ii) If $s\in\comp\tau\sigma$, $\zeta_{A\implies C}(s)=O$ and $sm_1,sm_2\in\comp\tau\sigma$ for some $m_1,m_2\in M_{A\implies C}$, then $m_1=m_2$.

    Claim (ii) is what we are trying to prove.  We prove (i) and (ii) by simultaneous induction on the length $n$ of $s$.  

    If $s$ is the empty sequence, then $s$ is covered by $\emptyplay\in\sigma\|\tau$, which is clearly minimal.  Now suppose that $n\ge1$, so $s=s'm$ for some $s'\in\comp\tau\sigma,m\in M_{A\implies C}$.  By the inductive hypothesis (i), there is some $\s'$ minimal among sequences in $\sigma\|\tau$ that cover $s'$.  

    Suppose first that $\zeta_{A\implies C}(s')=P$.  Suppose that $m=a\in M_A$.  We claim that $\s'\vert_{A,B}a\in P_{A\implies B}$.  $\s' a\vert_{A,C}=s'a\in P_{A\implies C}$, while $\s' a\vert_{B,C}=\s'\vert_{B,C}\in\tau\subset P_{B\implies C}$.  Therefore, by Lemma \ref{alternatingsequenceslemma}, $\s'\vert_{A,B}a=\s' a\vert_{A,B}\in P_{A\implies B}$.  Since $\s'\vert_{A,B}\in\sigma$ and $\lambda_{A\implies B}(a)=\neg\lambda_A(a)=\lambda_{A\implies B}(a)$, we must have $\s'\vert_{A,B}a\in\sigma$, by the definition of a strategy.  Therefore, $\s'a\in\sigma\|\tau$.
    
    Moreover, $\s'a$ is minimal among sequences that cover $s'a$: any such sequence must cover $s'$ and so must have $\s'$ as a prefix by minimality of $\s'$; moreover, the next move after $\s'$ must be a move in $A$, since the opponent is not permitted to switch games.  $\s'a$ is clearly minimal among sequences that have these properties and also include the move $a$.

    An identical argument applies in the case that $m=c\in M_C$.

    Now suppose that $\zeta_{A\implies C}(s')=O$.  Let $\s$ be any sequence covering $s$; after removing $B$-moves from the end of $\s$, we may assume that $m$ is the last move in $\s$.  We claim that $\s$ is minimal among sequences covering $s$.  

    Since $\s$ covers $s'$, we must have $\s'\prefix\s$ by minimality of $\s'$.  Since $m$ is the last move in $\s$, we have
    \[
      \s=\s'b_1\dots b_km
      \]
    for some $k\ge0,b_1,\dots,b_k\in M_B$.  

    Now let $m'\in M_{A\implies C}$, and suppose that $s'm\in\comp\tau\sigma$.  Let $\t$ be some sequence covering $s'm$.  Again, after removing $B$-moves from the end of $\t$, we may assume that $m$ is the last move of $\t$, and as before we may write $\t$ as
    \[
      \t=\s'\beta_1\dots \beta_jm'
      \]
    for some $j\ge0,\beta_1,\dots,\beta_j\in M_B$.  We shall show that $j=k$, that $\beta_i=b_i$ for all $i=1,\dots,j$ and that $m=m'$.  This will prove claims (i) (by setting $m'=m$) and (ii) for us.  

    We have $\zeta_{A\implies C}(\s'\vert_{A,C})=\zeta_{A\implies C}(s')=O$, so $\zeta_A(\s'\vert_A)=P$ and $\zeta_C(\s'\vert_C)=O$.  Let us suppose for now that $\zeta_B(\s'\vert_B)=P$; the other case is entirely similar.  In that case, $\zeta_{A\implies B}(\s'\vert_{A,B})=P$ and $\zeta_{B\implies C}(\s'\vert_{B,C})=O$.  Since $\s'\vert_{B,C}b_1,\s'\vert_{B,C}\beta_1\in\tau$, we must have $b_1=\beta_1$.  Now, by the alternating condition, we must have $\zeta_{A\implies B}(\s'b_1\vert_{A,B})=O$ and $\zeta_{A\implies B}(\s'b_1\vert_{B,C})=P$.  Since $\s'b_1\vert_{A,B}b_2,\s'b_1\vert_{A,B}\beta_2\in\sigma$, we must have $b_2=\beta_2$.  This process continues until we reach the first move outside $M_B$, which must be $m$ in $\s$ and $m'$ in $\t$.  Since $m,m'$ are both $P$-moves, they must both occur as $P$-replies to the same $O$-play in one of $\sigma$ or $\tau$, so $m=m'$.
  \end{proof}
\end{proposition}
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    Now suppose we have constructed a non-empty sequence $\ss'm$ such that $\ss'\vert_{A,B,D}\prefix\s$ and $\ss'\vert_{B,C,D}\prefix\t$.  

    If $m=a\in M_A$, then the prefix $\ss'\vert_{A,B,D}\prefix\s$ also ends in $a$.  If $\ss'\vert_{A,B,D}=\s$, then set $\ss=\ss'$ and finish.  Otherwise, extend $\ss'$ by appending the next move that occurs in $\s$ after $\ss'\vert_{A,B,D}$.  If $\lambda_{A\implies B}(a)=P$, then the next move in $\s$ must also take place in $A$, by the proof of Proposition \ref{WellDefinedComposition}.  Therefore, restricting our new sequence to $B,C,D$ gives us $\ss'\vert_{B,C,D}$, which we already know is a prefix of $\t$.  
  \end{proof}
\end{proposition}
