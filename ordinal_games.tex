...
For example, let $\st$ be the negative game where each player has a unique move at every position:
\begin{gather*}
  M_{\st} = \{q,a\} \\
...
Similarly, player $P$ wins an infinite play $s$ in $A\implies B$ if and only if $s\vert_A$ is infinite and $O$-winning or if 
...
We want to extend the definitions and results from the previous section 
