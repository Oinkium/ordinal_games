...
\begin{proposition}
  ...
  \end{enumerate}
  Furthermore, (i) and (ii) hold if we replace $\tensor$ with $\implies$ throughout and if we change the first second `$P$' to an `$O$' in part (i).
\end{proposition}
...
\begin{proof}[Proof of Proposition \ref{NiceAcnGames}]
  ...
  We now prove (i) and (ii) for all successor plays by induction on their length.  Let $sx\in P_A\|P_B$ be alternating.  Then by induction either $t\vert_A=P$ or $t\vert_B=P$, so there are three cases:
  \begin{description}
    \item[Case 1:] $\zeta_A(t\vert_A=P)$ and $\zeta_B(t\vert_B)=P$.  Then either $sx\vert_A=s\vert_A$ (if $x$ is a $B$-move) or $sx\vert_B=s\vert_B$ (if $x$ is an $A$-move.  This proves part (i).  For part (ii), note that in this case we have $\zeta_{A\tensor B}(s)=P$, so $\zeta_{A\tensor B}(sx)=O$, since $sx$ is alternating.  If $x$ is an $A$-move, it means that $\zeta_B(sx\vert_B)=\zeta_B(s\vert_B)=P$, so we must have $\zeta_A(sx\vert_A)=O$ by the definition of $\zeta_{A\tensor B}$.  Then
      \[
        \lambda_{A\tensor B}(x)=\lambda_A(x)=\zeta_A(s\vert_Ax)=O=\zeta_{A\tensor B}(sx)
        \]
      If $x$ is a $B$-move, the argument is similar.
  \end{description}
\end{proof}
