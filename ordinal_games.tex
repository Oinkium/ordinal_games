...
\[
  \sigma\|\tau = \{\s\in(M_A\cprd M_B\cprd M_C)^*\suchthat \s\vert_{A,B}\in \sigma,\s\vert_{B,C}\in\tau, \s\vert_{A,C}\in P_{A\implies C}\}\footnote{TODO: explain why this last condition is necessary}
  \]
We then define
\[
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    By the definition of $\sigma\|\tau$, it is clear that $\comp\tau\sigma\subset P_{A\implies C}$.  To show that it is a subset of $L_{A\implies C}$, it will suffice by alternation to show that $\zeta_{A\implies C}(\emptyplay) = P$.  The existence of the strategies $\sigma,\tau$ tell us that $\emptyplay\in L_{A\implies B}$ and $\emptyplay\in L_{B\implies C}$; it follows that $\zeta_{A\implies B}(\emptyplay)=\zeta_{B\implies C}(\emptyplay) = P$.  Then transitivity of $\Rightarrow$ tells us that $\zeta_{A\implies C}(\emptyplay) = P$.  

    To finish, we must show that the two strategy conditions hold for $\comp\tau\sigma$.  Firstly, suppose that $\s\in\sigma\|\tau$ and that $\zeta_{A\implies C}(\s\vert_{A,C})=P$, and suppose moreover that $m\in M_{A\implies C}$ is such that $\s\vert_{A,C}m\in P_{A\implies C}$.  We need to show that $\s\vert_{A,C}m\in\comp\tau\sigma$.  

    Certainly in this case we must have $\lambda_{A\implies C}(m)=O$.  
