...
\begin{remark}[Notes on the definition]
  ...
  Given the game $A$, we define $L_A$ to be the set of all plays $s\in P_A$ such that either $s$ has even length and $\zeta_A(s)=P$ or $s$ has odd length and $\zeta_A(s)=O$.  
\end{remark}
...
\begin{proposition}
  $\neggame A$, $A\tensor B$,$A\parr B$,$A\implies B$ are well formed games.
  \begin{proof}
    This is obvious for $\neggame A$.  We will prove the proposition for $A\tensor B$ and the other two cases are entirely similar.  Alternatively, observe that $A\parr B=\neggame{(\neggame A \tensor \neggame B)}$ and $A\implies B = \neggame A \parr B$.

    For $A\tensor B$, it suffices to show that $P_{A\tensor B}$ is alternating with respect to $\zeta_{A\tensor B}$.  Indeed, suppose $s,sa\in P_{A\tensor B}$.  Then $s,sa$ are both prefixes of $sa$.  Since $sa\in P_{A\tensor B}$, it must be alternating with respect to $\zeta_{A\tensor B}$ and therefore $s$ and $sa$ have opposite signs under $\zeta_{A\tensor B}$.  
  \end{proof}
\end{proposition}
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    We want to show that $\comp\tau\sigma\subset L_A$.  First, we show that $\zeta_{A\implies C}(\emptyplay)=P$.  Note that since $\sigma\in L_{A\implies B},\tau\in L_{B\implies C}$, we must have $\zeta_{A\implies B}(\emptyplay)=\zeta_{B\implies C}(\emptyplay)=P$.  It follows that $\zeta_{A\implies C}(\emptyplay)=P$ by transitivity of $\Rightarrow$.  

    We now show that $\comp\tau\sigma\subset P_{A\implies C}$.  If $\s\in\sigma\|\tau$ then $\s\vert_{A,B}\in\sigma\subset P_{A\implies C}$, so $\s\vert_A=\s\vert_{A,B}\vert_A\in P_A$.  Similarly, $\s\vert_C\in P_C$.  So $\s\vert_{A,C}\in P_A\| P_C$.  

    It remains to show that $\comp\tau\sigma$ is alternating with respect to $\zeta_{A\implies C}$.  This will also complete the proof that $\comp\tau\sigma\in L_{A\implies C}$.  Let $s\in\comp\tau\sigma$, and let $\s\in\sigma\|\tau$ be minimal such that $\s\vert_{A,C}=s$ (so if $s=\emptyplay$ then $\s=\emptyplay$ and if $s=s'y$ then $\s$ has $y$ as its last move).  

    We know that $\zeta_{A\implies C}(\emptyplay) = P$, so we first need to show that if $x\in M_{A\implies C}$ and $x\in \comp\tau\sigma$ then $\lambda_{A\implies C}(x)=O$.  First suppose that $x=a$ is an $A$-move.  
  \end{proof}
\end{theorem}
